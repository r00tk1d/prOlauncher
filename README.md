...
- **Center-aligned home screen**: apps are centered on the home screen by default (can be changed to left/right in Settings → App alignment).
- **Sectioned long-press menu**: the home slot menu groups options under *Add slot* (Add app, Add folder) and *Modify Slot* (Replace app, Rename app, Remove app) headers; long-pressing a folder shows *Add app* and *Add folder* to add to the home screen, plus a *Modify* section with *Add app inside folder*, *Rename folder*, and *Remove folder*.
